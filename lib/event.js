...
var storageKey = 'attendeeData-'+process.env.EB_EVENT_ID;
...
function poll() {
	return Promise.resolve()
		.then(function() {
			var opts = {method:'attendees', changed_since:lastAttendeeList.utcOffset(0).subtract(300, 'seconds')};
			lastAttendeeList = moment();
			return ebRequest(opts)
			.then(function(ebattendees) {
				if (ebattendees.length) {
					debug('Received attendee records: %d', ebattendees.length);
				}
