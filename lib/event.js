...
const poll = async () => {
  ...
  const attendees = data.map(attendee => ({
    ...
    displayName: (attendee.answers.find(o => o.question.match(/(display|badge) name/i)) || {}).answer || attendee.profile.first_name,
    pronoun: (attendee.answers.find(o => o.question.match(/pronoun/i) && o.answer !== "Don't show on badge") || {}).answer,
    affiliation: attendee.profile.company,
